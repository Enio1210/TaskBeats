...
class TaskDetailActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val edtDescriptor = findViewById<EditText>(R.id.edit_task_descripition)
        btnDone = findViewById<Button>(R.id.btn_done)

        if (task != null) {
            edtTitle.setText(task!!.title)
            edtDescriptor.setText(task!!.description)

        }

        btnDone.setOnClickListener {
            val title = edtTitle.text.toString()
            val desc = edtDescriptor.text.toString()
        ...
        //setar um novo text na tela
        //tvTitle.text=task?.title?:"adcione uma tarefa"
    }
